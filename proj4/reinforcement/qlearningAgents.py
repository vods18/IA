...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        legalActions = self.getLegalActions(state)
        maxqvalue = max(self.getQValue(state, action) for action in legalActions)
        bestAction = [action for action in legalActions if self.getQValue(state, action) == maxqvalue]
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.qvalue[key] = (1.0 - self.alpha) * current_q_value + self.alpha * sample
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        feature_Extractor = self.featExtractor
        features = feature_Extractor.getFeatures(state, action)
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        maxqnext = max((self.getQValue(nextState, act) for act in actionsFromNextState), default=0)
